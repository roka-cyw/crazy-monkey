// https://github.com/mileung/rn-sprite-sheet
...
export default class SpriteSheet extends React.PureComponent {
  ...
  constructor(props) {
    ...
    let { source, height, width, rows, columns, frameHeight, frameWidth, offsetY, offsetX } = this.props
  ...
  render() {
    let { imageHeight, imageWidth, frameHeight, frameWidth, animationType, offsetX, offsetY } = this.state
    let { viewStyle, imageStyle, source, onLoad } = this.props

    let { translateY = { in: [0, 0], out: [offsetY, offsetY] }, translateX = { in: [0, 0], out: [offsetX, offsetX] } } =
